...
def validate(val_loader, model, criterion, epoch):
    ...
    print(
        "   * EPOCH {epoch} | Accuracy: {acc.avg:.3f} | Loss: {losses.avg:.3f}".format(
            epoch=epoch, acc=acc, losses=losses
        )
    )
...
def save_checkpoint(state, is_best, savepath):
    torch.save(state, os.path.join(savepath, "checkpoint.pth"))
    if is_best:
        shutil.copyfile(
            os.path.join(savepath, "checkpoint.pth"),
            os.path.join(savepath, "model_best.pth"),
        )


def main(mode="train", resume=""):
    ...
    for arch in archs:
        ...
        # create model
        print(f"=> Starting {mode} on '{arch}' model")
...
if __name__ == "__main__":
    args = parse_args()
    main(mode=args.mode, resume=args.checkpoint_path)
